          ...
          settings =
            let
              common = {
                ...
                tray.icon-size = 21;
                tray.spacing = 10;
                "wireplumber#sink".node-type = "Audio/Sink";
                "wireplumber#sink".format = "{icon} {volume}%";
                "wireplumber#sink".format-icons.default = [
                  ""
                  ""
                ];
                "wireplumber#sink".format-muted = "";
                "wireplumber#sink".on-click = "wpctl set-mute @DEFAULT_AUDIO_SINK@ toggle";
                "wireplumber#sink".on-click-right = "${lib.getExe pkgs.myScripts.wpctl-cycle} sink";
                "wireplumber#source".node-type = "Audio/Source";
                "wireplumber#source".format = "󰍬 {volume}%";
                "wireplumber#source".format-muted = "󰍭";
                "wireplumber#source".on-click = "wpctl set-mute @DEFAULT_AUDIO_SOURCE@ toggle";
                "wireplumber#source".on-click-right = "${lib.getExe pkgs.myScripts.wpctl-cycle} source";
                "custom/brightness" = {
